...
function Header() {
  ...
  const handleDownload = async () => {
    /*Updates the form value when the user types*/
    try {
      const startDate = format(
        range[0].startDate,
        "yyyy-MM-dd",
      ); /*Takes the selected start date and formats it like: 2025-05-08.*/
      const endDate = format(
        range[0].endDate,
        "yyyy-MM-dd",
      ); /*Takes the selected end date and formats it the same way.*/
      const res = await fetch(
        `${apiBaseUrl}/api/sales/downloadpdf?start=${startDate}&end=${endDate}`,
      ); /*Sends a request to your backend to get the PDF file for the selected dates. */
      if (!res.ok)
        throw new Error(
          "Failed to download PDF",
        ); /*If the server did not respond correctly, stop and show an error.*/
      const blob =
        await res.blob(); /*Takes the server’s response and converts it into a file object (PDF)*/

      const url =
        window.URL.createObjectURL(
          blob,
        ); /*Creates a temporary download link for the PDF file.*/
      const a =
        document.createElement(
          "a",
        ); /*Creates a hidden download button in the browser.*/
      a.href = url;
      ...
      document.body.removeChild(
        a,
      ); /*Removes the button after clicking (clean-up).*/
